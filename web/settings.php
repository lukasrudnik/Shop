...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['name']) || 
            ...
            isset($_POST['surname']) || 
            strlen(trim($_POST['surname'])) >= 3 || 
            isset($_POST['address']) || 
            strlen(trim($_POST['address'])) >= 6 || 
            isset($_POST['email']) || 
        ...
        if (!empty($_POST['email']) . $connect -> real_escape_string($_POST['email']) &&
            ...
            if ($result == true) {
                echo 'Data changed correctly! Your new username is: ' . $_POST['name'] . '<br>';
            } else {
                echo 'Dane nie zostały zmienione <br>';
            }
        }
    }
}
